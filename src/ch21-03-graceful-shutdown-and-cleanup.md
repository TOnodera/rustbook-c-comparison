...
このエラーは、各`worker`への可変参照しか持っていない一方で、`join`が呼び出し先の所有権を必要とするため、`join`を呼べないと示しています。問題を解決するには、`join`がスレッドを消費できるよう、`thread`を所有する`Worker`から値をムーブする必要があります。

方法の一つは、リスト18-15で使ったのと同じやり方です。`Worker`が`Option<thread::JoinHandle<()>>`を保持していれば、`Option`の`take`メソッドを呼び出し、`Some`列挙子から値をムーブして、その場所に`None`を残せます。つまり、実行中の`Worker`では`thread`が`Some`になり、`Worker`を片付けるときに`Some`を`None`へ置き換えれば、その`Worker`には実行するスレッドがなくなります。
...
まず、`ThreadPool`の`drop`実装を変更し、ワーカースレッドの終了を待つ前に`sender`を明示的にドロップします。
